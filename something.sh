...
HD=$(df -hl /dev/nvme0n1p2 | grep nvme0n1p2| awk '{printf ("%02d", $5)}' | sed -e 's/%//')
[ $DEBUG -eq 1 ] && echo "DEBUG: hd -> "$HD
HDUSED=$(df -hl /dev/nvme0n1p2 | grep sda1 | awk '{print $3}' | sed -e 's/G//')
HDSIZE=$(df -hl /dev/nvme0n1p2| grep sda1 | awk '{print $2}' | sed -e 's/G//')
[ $DEBUG -eq 1 ] && echo "DEBUG: hd size -> $HDSIZE GB drive capacity"
TOPHD=$(df -hl /dev/nvme0n1p2| tail -1)
